...
def getIO(root, name):
    "Get a digital input or output port to the given name from the root of ElementTree" 
    foundIO = 0    
    for io in root.findall('DigitalIn'):    # Check if name occurs in digital inputs
        if io.get('name') == name:          # if yes,   
            result = io.get('channel')      # get the PLC-port
            foundIO = 1
            break
    if foundIO == 0:                        # if not found yet,
        for io in root.findall('DigitalOut'):   # keep searching within outputs
            if io.get('name') == name:
                result = io.get('channel')
                foundIO = 1
                break
    if foundIO == 0:    # Exception in case the name is not found. Should not happen if program compiles without errors in JGraphchart
        raise NameError, name
    return result

# test for getIO (remove)
test = getIO (root, 'Lampe')
print test
