...
def generateSCL(filename, stepList, transitionList, digInDict, digOutDict):
    ...
    outfile.write ('(* File autogenerated by graf2il.py V0.4 *)\n\n')
    ...
    def findStepN(n, sList):
        "Find tuple step from unsorted stepList with index n"
        for step in sList:
    ...
    def findInputNameFromCondition(condition, inDict):
        "Find variable name of input from conditions from the dictionary"
        for name in inDict:
            if inDict[name] == condition:
                break
        return (name)       
    ...
    for thisTransition in transitionList:
        ...
        fromStep = findStepN(fromStepn, stepList)   # Get full info about fromStepn
        fromStepName = fromStep[1]                  # Name is in element 1.

        conditionVariableName = findInputNameFromCondition (condition, digInDict)
    ...
    for thisTransition in transitionList:
        ...
        outfile.write ('    IF (#{0} OR #{1})'.format(name, toStepName))
        
        # Add the following transitions as "AND NOT"        
